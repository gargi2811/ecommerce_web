...
const Cart: FC = () => {
  ...
  const [copon, setCopon] = useState<{ text: string; percent: number }>({
    text: "",
    percent: 0,
  });
    ...
    if (copon.percent <= 0) {
      for (const cop in coponData) {
        if (cop === values.copon) {
          ...
          setStorage("DISCOUNT_COPON", {
            text: cop,
            percent: coponData[cop as ObjectKey],
          });
          const coponDiscount: any = (
            (totalProductsPrice * coponData[cop as ObjectKey]) /
            100
          ).toFixed(2);
      ...
      if (!coponData.hasOwnProperty(values.copon)) {
        errorToast(
          "copon code is not correct",
          'try " TEST " to get 20% discount'
        );
      }
    } else {
        ...
        <div className="cart-page__right">
          <div className="cart-page__right__summery-box">
            ...
            <p className="cart-page__right__summery-box__discription">
              you can use{" "}
              <span className="cart-page__test-important">TEST</span> copon as
              test copon to see the discount system
            </p>
            ...
            <h4 className="cart-page__right__summery-box__discount-status">
              {copon.percent > 0
                ? `${copon.percent}% discount and you save ${discount} ₹`
                : ""}
            </h4>
        ...
        <div className="cart-page__left">
          {cart.length <= 0 && (
            <>
              ...
              />
              <h4 className="cart-page__left__empty-cart__title">
